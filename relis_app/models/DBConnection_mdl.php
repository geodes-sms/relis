...
class DBConnection_mdl extends CI_Model
{
    ...
    function save_reference_mdl($content, $type = 'normal')
    {
        ...
        $raw_fields = $this->db3->query("SHOW COLUMNS FROM " . $table_config['table_name'])->result_array();
        $fields = array();
        ...
        foreach ($table_config['operations'][$current_operation]['fields'] as $key => $v_field) {
            ...
            $is_nullable = $fields[$key]["Null"] == "YES";
            ...
            if ($v_field['field_state'] != 'drill_down' and $v_field['field_state'] != 'disabled' and !((isset($value['multi-select']) and isset($value['multi-select']) == 'Yes'))) {
                ...
                $sql_value = $this->get_sql_value($value, $val, $is_nullable, $this->db3->conn_id);
                if ($i == 0) {
                    $param .= $sql_value;
                } else {
                    $param .= ", " . $sql_value;
   ...
   function get_subcategory_resolved_values($parent_table, $sub_field, $intermediate, $element_id)
     ...
     */
    private function get_sql_value($value, $val, $is_nullable, $db_conn)
    {
        if (
            $is_nullable && (
