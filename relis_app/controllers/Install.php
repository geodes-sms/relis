...
class Install extends CI_Controller
{
	...
	/**
	 * Open the ReLiS Editor
	 */
	public function relis_editor()
	{
	...
	/**
	 * Installation form when updating project from a php file 
	 */
	...
	/**
	 * Installation form when updating project from the syntax written in the Editor
	 */
	public function install_form_editor()
	{
		...
		//read workspace forder to see generated installation files
				...
				if (is_dir($dir . $path_separator . $file)) {
					...
					foreach ($project_content as $key => $value_c) {
						if (!is_dir($project_dir . $path_separator . $value_c)) {
							...
							$project_content_gen = array_diff(scandir($project_dir . $path_separator . 'src-gen'), array(
									'.',
									'..',
									".metadata"
							));
	...
	/**
	 * Installation form when new project from a php file
	 */
	public function new_project()
	{
		...
		$data['top_buttons'] .= get_top_button('back', 'Back', 'manager/projects_list/');
	...
	/**
	 * Installation form when new project from the syntax written in the Editor
	 */
	public function new_project_editor()
	{
				...
				if (is_dir($dir . $path_separator . $file)) {
					...
					foreach ($project_content as $key => $value_c) {
						if (!is_dir($project_dir . $path_separator . $value_c)) {
							...
							$project_content_gen = array_diff(scandir($project_dir . $path_separator . 'src-gen'),
									array('.','..',".metadata"));
							foreach ($project_content_gen as $key_g => $value_g) {
								if (!is_dir($project_dir . $path_separator . 'src-gen' . $path_separator . $value_g)) {
									array_push($Tprojects[$file]['generated'], $value_g);
		...
		$data['top_buttons'] .= "<li>" . anchor('install/relis_editor', '<button class="btn btn-primary">  ' . lng_min('Open editor') . ' </button></li>', 'title="' . lng_min('Open editor') . '" ') . "</li>";
		$data['top_buttons'] .= get_top_button('back', 'Back', 'manager/projects_list/');
	...
	/**
	 * Handle new project installation request after uploading an installation file
	 */
	...
	/**
	 * Handle new project installation request after selecting the generated installation file
	 * From editor
	 */
	public function save_new_project_editor()
	{
		...
		if (empty($post_arr['selected_config'])) {
			...
			if (empty($Tline[1])) {
				...
				$project_short_name = trim($Tline[1]);
				//Verify if the project is already installed
				$sql                = "SELECT project_id from projects where project_label LIKE '" . $project_short_name . "' AND project_active=1 ";
				$resul              = $this->db->query($sql)->result_array();
				//print_test($resul);
				if (!empty($resul)) {
	...
	public function save_new_project_part2($project_short_name, $verbose = false)
	{
		...
		//create initial tables stored procedures and Views
		...
		if (!empty($res_install_config['project_title']))
			...
			//adding Quality assessment values
	...
	/**
	 * Handle updating project request after uploading an installation file
	 */
	public function save_install_form()
	{
		...
		if ($_FILES["install_config"]["error"] > 0) {
			...
			if (!empty($Tline[1]) AND trim($Tline[1]) == project_db()) {
				...
				fclose($f_new);
				$temp_table_config = $this->entity_configuration_lib->get_new_install_config(project_db());
				if (!valid_install_configuration_file($temp_table_config)) {
					array_push($error_array, "Not a valid configuration file");
				} else {
	...
	/**
	 * Handle updating project request after selecting the generated installation form
	 */
	public function save_install_form_editor()
	{
		...
		if (empty($post_arr['selected_config'])) {
			...
			if (!empty($Tline[1]) AND trim($Tline[1]) == project_db()) {
				...
				fclose($f_new);
				$temp_table_config = $this->entity_cconfiguration_lib->get_new_install_config(project_db());
				if (!valid_install_configuration_file($temp_table_config)) {
					array_push($error_array, "Not a valid configuration file");
				} else {
					copy($project_specific_config_folder . "temp/install_config_" . project_db() . ".php", $project_specific_config_folder . "install_config_" . project_db() . ".php");
	...
	/**
	 * Validation page before uninstalling a project
	...
	function remove_project_validation($project_id)
	{
		...
		$data['array_warning']         = array(
				'You want to unistall the project : ' . $detail_project['project_title'] . '  .The opération cannot be undone !'
		);
		...
		$data['next_operation_button'] .= " &nbsp &nbsp &nbsp" . get_top_button('all', 'Cancel', 'manager/projects_list', 'Cancel', '', '', ' btn-danger ', FALSE);
		$this->load->view('body', $data);
	}
	...
	/**
	 * Populate the database after creating a project (Tables Views and Storeed procedures)
	 * 
	 * @param string $project_short_name : project_label 
	 */
	private function populate_created_database($project_short_name)
	{
		$this->db2 = $this->load->database($project_short_name, TRUE);
		$db_sql    = file_get_contents("relis_app/libraries/table_config/init_sql/project_initial_query.sql");
	...
	/**
	 * Add in CI database configuration the credentiale to a project database
	 *
	 * @param string $project_short_name : project_label
	 */
	...
	/**
	 * Clean previous installation values before updating project configuration
	 */
...
('" . $ref_conf . "', '" . $ref_conf . "', '" . $desc . "', 1);";
	...
	private function update_screening_values($screening, $target_db = 'current')
	{
		...
		if (!empty($screening['phases'])) {
			//clean existing
			$result     = $this->db3->update('screen_phase', array(
					'screen_phase_active' => 0
			));
			//get total number
		...
		$screen_configs_to_save = array(
				'exclusion_criteria' => array(
						'table' => 'ref_exclusioncrieria'
				),
				'source_papers' => array(
						'table' => 'ref_papers_sources'
				),
				'search_startegy' => array(
						'table' => 'ref_search_strategy'
				)
		);
		...
		foreach ($screen_configs_to_save as $s_config_id => $s_config_value) {
			if (!empty($screening[$s_config_id])) {
				//clean existing
				$result       = $this->db3->update($s_config_value['table'], array(
						'ref_active' => 0
				));
				$all_elements = array();
	...
	private function update_qa_values($qa, $target_db = 'current')
	{
		...
		if (!empty($qa['questions'])) {
			$all_phases = array();
			$i          = 1;
			$result     = $this->db3->update('qa_questions', array(
					'question_active' => 0
			));
		...
		if (!empty($qa['responses'])) {
			$all_responses = array();
			$i             = 1;
			$result        = $this->db3->update('qa_responses', array(
					'response_active' => 0
			));
			$conf_phase    = array();
