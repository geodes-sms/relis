...
class Screening extends CI_Controller
{
    ...
    public function save_phase_screen()
    {
        ...
        //new changes for making each phase title unique
        ...
        foreach ($existing_phase_titles as $title) {
            if ($post_arr['phase_title'] === $title) {
                ...
                $phase_title_exists = true;
                break;
            }
        }
        
        if ($phase_title_exists) {
            // Handle the case where the phase_title is not unique (e.g., return an error or throw an exception)
                   

            //this works        
            // echo '<script>alert("Note: Phase title should be unique");</script>';

            // // Delay the redirection by a few seconds (e.g., 3 seconds)
            // echo '<meta http-equiv="refresh" content="0;url=/element/add_element/add_screen_phase.html">';
            // exit;
            //till here

            //trying new soln
            $other_check = false;
            $data['err_msg'] .= lng(' Phase title should be unique ! ') . ' <br/>';
        }
